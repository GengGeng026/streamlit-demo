    ...
    with st.container() as config_container:
        ...
        if st.button("保存配置"):
            ...
            if st.session_state.config_saved:
                # Hide input fields and button, show success message
                config_container.markdown("<style>.stTextInput { display: none; }</style>", unsafe_allow_html=True)
                st.success("配置已成功保存。")
